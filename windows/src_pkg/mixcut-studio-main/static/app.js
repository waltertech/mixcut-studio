(() => {
  ...
  function appendMusicOrder(root, batch, item) { const track=document.createElement('div');track.className='track music-track';const h=document.createElement('h4');h.textContent='完整歌曲顺序';const editable=batch.status==='draft' && (item.music?.length||0)>1;const hint=document.createElement('p');hint.className='music-order-help';hint.textContent=editable?'拖动歌曲，或使用箭头调整；修改会立即保存。':'歌曲顺序已锁定。';const list=document.createElement('ol');list.className='music-order';const move=(from,to)=>{if(from===to||to<0||to>=item.music.length)return;const ids=item.music.map(song=>song.id);const [value]=ids.splice(from,1);ids.splice(to,0,value);saveMusicOrder(batch,item,ids);};(item.music||[]).forEach((song,index)=>{const li=document.createElement('li');li.dataset.index=String(index);li.draggable=editable;const handle=document.createElement('span');handle.className='drag-handle';handle.textContent=editable?'⠿':'•';handle.title=editable?'拖动调整顺序':'';const name=document.createElement('span');name.className='music-name';name.textContent=song.name||song.path||song.asset_id||String(song);li.append(handle,name);if(editable){const controls=document.createElement('span');controls.className='order-buttons';[['↑',index-1,'上移'],['↓',index+1,'下移']].forEach(([label,target,title])=>{const button=document.createElement('button');button.type='button';button.textContent=label;button.title=title;button.disabled=target<0||target>=item.music.length;button.onclick=()=>move(index,target);controls.append(button);});li.append(controls);li.addEventListener('dragstart',event=>{event.dataTransfer.effectAllowed='move';event.dataTransfer.setData('text/plain',String(index));li.classList.add('dragging');});li.addEventListener('dragend',()=>li.classList.remove('dragging'));li.addEventListener('dragover',event=>{event.preventDefault();event.dataTransfer.dropEffect='move';li.classList.add('drag-over');});li.addEventListener('dragleave',()=>li.classList.remove('drag-over'));li.addEventListener('drop',event=>{event.preventDefault();li.classList.remove('drag-over');move(Number(event.dataTransfer.getData('text/plain')),index);});}list.append(li);});track.append(h,hint,list);root.append(track); }
