...
def import_image(data: bytes, name: str, root: Path) -> dict[str, Any]:
    ...
    if not (data.startswith(b"\x89PNG\r\n\x1a\n") or data.startswith(b"\xff\xd8\xff") or data.startswith(b"RIFF") and data[8:12] == b"WEBP"):
        raise ValueError("仅支持 PNG、JPG 或 WEBP 位图文件")
    ...
    with tempfile.TemporaryDirectory(dir=root) as temporary:
        ...
        if not video or video.get("codec_name") not in _CODECS:
            raise ValueError("仅支持静态 PNG、JPG 或 WEBP 贴纸")
        ...
        # More than one image frame means animated WebP (or another non-static input).
...
def add_overlay_filters(inputs: list[str], filters: list[str], layers: list[dict[str, Any]], input_index: int,
    ...
    for offset, layer in enumerate(layers):
        ...
        filters.append(f"[{input_index + offset}:v]format=rgba,scale={actual_width:.4f}:{actual_height:.4f},colorchannelmixer=aa={float(layer['opacity']):.6f}[{image}]")
